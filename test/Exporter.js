...
test('save a panel with only 1 standard component', t => {
  ...
  t.is(exporter.data.length, 1)
  t.is(exporter.data[0].value, 10)
  t.not(exporter.data[0].uid, undefined)
})
...
test('save a panel with only 1 color component', t => {
  ...
  t.is(c.uid.split('-').length, 2)
  t.is(exporter.data.length, 1)
  t.not(exporter.data[0].value, undefined)
  t.not(exporter.data[0].uid, undefined)
})
...
test('save a panel with 3 components of differents types: number, string and color', t => {
  ...
  t.is(exporter.data.length, 3)

  t.is(exporter.data[0].type, 'color')
  t.is(exporter.data[1].type, typeof c2.value)
  t.is(exporter.data[2].type, typeof c3.value)

  exporter.data.forEach((obj, i) => {
...
test('export saved panel with 2 components in json format', t => {
  ...
  const json = exporter.dataToJson()
  const fileHref = exporter.exportToJson()
